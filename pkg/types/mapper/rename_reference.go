...
func (r *RenameReference) ModifySchema(schema *types.Schema, schemas *types.Schemas) error {
	...
	for name, field := range schema.ResourceFields {
		if definition2.IsReferenceType(field.Type) && strings.HasSuffix(name, "Name") {
			newName := strings.TrimSuffix(name, "Name") + "Id"
			newCodeName := convert2.Capitalize(strings.TrimSuffix(name, "Name") + "ID")
		...
		} else if definition2.IsArrayType(field.Type) && definition2.IsReferenceType(definition2.SubType(field.Type)) && strings.HasSuffix(name, "Names") {
			newName := strings.TrimSuffix(name, "Names") + "Ids"
			newCodeName := convert2.Capitalize(strings.TrimSuffix(name, "Names") + "IDs")
			move := Move{From: name, To: newName, CodeName: newCodeName}
			if err := move.ModifySchema(schema, schemas); err != nil {
				return err
			}
